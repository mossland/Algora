# Algora Multi-Agent Upgrade Proposal

> Inspired by oh-my-opencode's Multi-Agent Orchestration Patterns
...
### 현재 구조
- **30개 에이전트**: 7개 클러스터 (Visionaries, Builders, Investors, Guardians, Operatives, Moderators, Advisors)
- **라운드 로빈 참여**: 무작위 선택 기반
- **단일 세션 처리**: 순차적 라운드 진행 (30-120초/턴)
- **3-Tier LLM**: Tier 0 (무료), Tier 1 (로컬), Tier 2 (외부 API)
...
### 2.1 분리된 전문 에이전트 (Separation of Concerns)
...
│     ├── Prometheus (Planner) ─── 계획만 작성, 실행 안함        │
│     ├── Oracle (Debugger) ───── 고수준 분석                   │
│     ├── Librarian (Researcher) ─ 증거 기반 조사               │
│     ├── Explore (Search) ────── 코드베이스 탐색               │
...
### 3.1 아키텍처 재설계: 7-Agent Core System

기존 30개 에이전트를 7개 핵심 역할로 재편:

```
┌─────────────────────────────────────────────────────────────┐
│                  Algora 2.0 Agent Architecture              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   ┌─────────────┐                                           │
│   │   HERMES    │  Master Orchestrator (Claude Opus)        │
│   │ Orchestrator│  - 세션 관리, 에이전트 위임, 합의 조율       │
│   └──────┬──────┘                                           │
│          │                                                  │
│   ┌──────┼──────────────────────────────────────────┐       │
...
| Agent | Role | Tier | Permissions | Description |
|-------|------|------|-------------|-------------|
| **HERMES** | Orchestrator | 2 (Opus) | Read/Write | 마스터 오케스트레이터. 태스크 위임, 세션 관리, 최종 합의 조율 |
| **ATHENA** | Analyst | 2 (Sonnet) | Read-only | 신호 분석, 이슈 분류, 영향도 평가 |
| **ARGUS** | Guardian | 2 (Sonnet) | Read-only | 보안/규정 위험 평가, 컴플라이언스 체크 |
| **MNEMOSYNE** | Archivist | 1 (Haiku) | Read-only | 과거 결정 조회, 유사 사례 검색, 맥락 제공 |
| **DELPHI** | Council | 1 (Local) | Read-only | 다관점 토론 시뮬레이션 (30개 페르소나 통합) |
| **PROMETHEUS** | Advocate | 2 (Opus) | Write (proposals only) | 제안서 작성, 옵션 제시, 트레이드오프 분석 |
| **CALLIOPE** | Scribe | 2 (Sonnet) | Write (docs only) | Decision Packet 작성, 요약, 문서화 |
...
class BackgroundTaskManager {
  ...
  // Session persistence (like boulder.json)
  async saveSession(sessionId: string): Promise<void>;
  async restoreSession(sessionId: string): Promise<BackgroundTask[]>;
}
```

### 3.3 hermes_task Tool (sisyphus_task 적용)
...
### 3.4 Todo Continuation Enforcer
...
class TodoContinuationEnforcer {
  ...
  private async injectContinuationPrompt(
    ...
    const prompt = `
<system-reminder>
You have ${pendingTodos.length} pending tasks to complete:
${pendingTodos.map((t, i) => `${i + 1}. ${t.content}`).join('\n')}

Continue working on these tasks. Do not stop until all are completed.
</system-reminder>`;
...
### 3.6 Evidence-Based Claims System
...
// MNEMOSYNE 에이전트 응답 예시
const archivistResponse = `
<claim>
이전 유사 제안이 2025-08에 거부됨
</claim>
<evidence type="blockchain" tx="0x1234...abcd">
...
### 3.7 DELPHI Council: 30 Personas in One
...
class DelphiCouncil {
  ...
  private async simulateDebate(perspectives: Perspective[]): Promise<Debate> {
    // 로컬 LLM으로 빠른 다관점 토론 시뮬레이션
    const prompt = `
You are simulating a governance council debate among ${this.personas.length} members.

Perspectives:
${perspectives.map(p => `- ${p.persona}: ${p.position}`).join('\n')}

Generate a realistic debate with:
1. Key points of agreement
2. Major disagreements
3. Proposed compromises
4. Potential risks identified by each faction

Format response as structured XML.
`;

    return await this.llm.generate(prompt, { model: 'ollama-local', temp: 0.7 });
  }
}
```

### 3.8 Context Injection System

```typescript
// 신규: packages/agentic-consensus/src/context/ContextInjector.ts

interface InjectionRule {
  trigger: string;           // 매칭 패턴
  inject: string[];          // 주입할 파일/컨텍스트
  priority: number;
}
...
const INJECTION_RULES: InjectionRule[] = [
  ...
  {
    trigger: 'partnership|collaboration',
    inject: ['governance/PARTNERSHIP_GUIDELINES.md'],
    priority: 2
  }
];
...
class ContextInjector {
  async injectContext(issue: Issue, agentId: string): Promise<string> {
    ...
    context += await this.loadFile('.algora/AGENT_GUIDELINES.md');

    // 2. 이슈 기반 조건부 주입
    for (const rule of INJECTION_RULES) {
      if (new RegExp(rule.trigger, 'i').test(issue.title + issue.description)) {
    ...
    // 3. 에이전트별 전문 컨텍스트
    context += await this.loadAgentContext(agentId);
...
### Phase 1: Core Orchestration (1-2주)
...
### Phase 2: Specialized Agents (2-3주)
...
### Phase 3: Quality Assurance (1-2주)
...
```
Week 1-2: Hermes + BackgroundTaskManager 도입
          기존 AgoraService와 병행 운영

Week 3-4: ATHENA, ARGUS, MNEMOSYNE 구현
          기존 Operative/Guardian 에이전트 대체

Week 5-6: DELPHI Council 구현
          기존 30 에이전트 페르소나 통합

Week 7:   PROMETHEUS, CALLIOPE 구현
          전체 파이프라인 통합

Week 8:   기존 시스템 deprecated
          완전 전환
```
...
```typescript
// 기존 API 호환 래퍼
class LegacyAgoraAdapter {
  private hermes: HermesOrchestrator;

  // 기존 메서드 시그니처 유지
  async createSession(title: string, agentIds: string[]): Promise<Session> {
    // 내부적으로 Hermes 사용
    return this.hermes.orchestrate({
      type: 'deliberation',
      title,
      participantHint: agentIds.length
    });
  }
}
```
...
1. **30개 에이전트 → 7개 핵심 에이전트 + DELPHI Council**로 단순화
...
- [Algora Architecture](./ARCHITECTURE.md)
- [Agent System Spec](./ALGORA_PROJECT_SPEC.md)
